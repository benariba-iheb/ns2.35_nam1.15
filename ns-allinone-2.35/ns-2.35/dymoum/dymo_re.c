...
extern int VRTLength;       //lokmane added this
int decision = 1;          //lokmane added this
...
void NS_CLASS re_process(RE *re, struct in_addr ip_src, u_int32_t ifindex)
{
	...
	{
		decision = Forward_Decision(VRTLength,5);  //lokmane added this
		//printf("for RTL = %d the decision is: %d\n", VRTLength,decision);
		if (!no_path_acc)
		{
			...
			if (re->a)
				...
				//--------------end lokmane's block-------------------
				//re_forward_rreq_path_acc(re, n);  //lokmane comment this original DYMO line
			...
			//--------------end lokmane's block-------------------
			//re_forward(re);    //lokmane comment this original DYMO line
		}
	}
}
...
//lokmane added this function
//D represents the minimum density
//N represents the number of valid routes in the the routing table
int NS_CLASS Forward_Decision (int N,int D) { 
    //float C = 0.5;       //control factor for P
	float C = 0.1;       //control factor for P
    float R = ((float)rand() / RAND_MAX) * 100;  //Random number between 0 and 100
    float P;   //Message forwarding probability
    ...
    if (N <= D) {
        ...
        //P = ((float)100 / N) * (D * C);
        P = (float)(1000*C)/(N+D);
		//P = ((float)50/N)*(D+pow(C,2));
		//printf("RTL = %d, P = %f, R = %f\n",N, P,R);
        if (R < P) {
            return 1;
        } else {
            return 0;
