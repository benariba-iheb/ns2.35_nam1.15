...
void NS_CLASS hello_send(void *arg)
{
	...
	node_speed = ((MobileNode *) iNode)->speed(); 
 	hello_adaptive = hello_ival; 

	if (node_speed >= 5 && node_speed < 10){
	 	hello_adaptive = 5.0 / node_speed;
	}
	else if (node_speed >= 10){
		hello_adaptive = 0.5;
	}
		
	//fprintf(stderr, "node_speed: %d.\t\t noded_id: %d.\t\t hello_adaptive: %f\n", node_speed, ra_addr_, hello_adaptive);
	...
	// Schedule next HELLO
	timer_set_timeout(&hello_timer, (hello_adaptive*1000) + hello_jitter()); // modified by IHEB 
	//timer_set_timeout(&hello_timer, (hello_ival*1000) + hello_jitter()); 		//original
